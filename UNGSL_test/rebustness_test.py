...
def rebustness_test(data,miss_ratio):#鲁棒性分析
    device=data.x.device
    num_edge=data.edge_index.size(1)
    remove_num=int(miss_ratio*num_edge)
    remove_idx=np.random.choice(num_edge,remove_num,replace=False)

    mask=torch.ones(num_edge,dtype=torch.bool,device=device)
    mask[remove_idx]=False
    remaining_edge=data.edge_index[:,mask]
    perturbed=Data(x=data.x,edge_index=remaining_edge,y=data.y)
    return perturbed
...
for data in dataset:
    data=data.to(primary_device)
    x_data=rebustness_test(data,0.3)
    models = perform_5_fold_cv(x_data)
