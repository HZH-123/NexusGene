...
def shap_analysis(data, model, edge_index):  # 计算shap
    device = data.x.device
    model = model.to(device)
    x = data.x.cpu().numpy()
    omics_types = ["SNV", "METH", "GE", "CNA"]
    if x.shape[1] != len(omics_types):
        raise ValueError(f"期望数据有{len(omics_types)}列(每种组学一列)，但实际有{x.shape[1]}列")
    column_names = omics_types
    df_multi_omics = pd.DataFrame(x, columns=column_names)
    values = df_multi_omics.values
    class ModelWrapper(torch.nn.Module):
        def __init__(self, original_model, edge_index):
            super().__init__()
            self.model = original_model
            self.edge_index = edge_index.to(device)
        def predict(self, x):
            # 将numpy数组转换回tensor
            x_tensor = torch.FloatTensor(x).to(data.x.device)
            with torch.no_grad():  # 禁用梯度以节省内存
                return self.model(x_tensor, self.edge_index).cpu().numpy()
    wrapped_model = ModelWrapper(model, edge_index)
    explainer = shap.KernelExplainer(
        wrapped_model.predict,
        shap.sample(values)
    )
    shap_values = explainer.shap_values(values)
    shap_sum = np.abs(shap_values).mean(axis=0)  # 取绝对值后求均值
    omics_shap = {
        omics: shap_sum[i] for i, omics in enumerate(omics_types)
    }
    return omics_shap

def SNV_split(x):#组学数据分拆
    x=x.cpu().numpy()
    x=x[:,:16]
    row_mean=np.mean(x,axis=1)
    sorted_idx = np.argsort(row_mean)
    split_point = int(sorted_idx.shape[0] * 0.2)
    train_idx = sorted_idx[int(split_point):]
    test_idx = sorted_idx[:int(split_point)]
    return train_idx, test_idx
def GE_split(x):#组学数据分拆
    x=x.cpu().numpy()
    x=x[:,32:48]
    row_mean=np.mean(x,axis=1)
    sorted_idx = np.argsort(row_mean)
...
def perform_10_fold_cv(data):
    train_idx, test_idx = SNV_split(data.x)
    #train_idx, test_idx = GE_split(data.x)
    #train_idx, test_idx = METH_split(data.x)
    #train_idx, test_idx = CNA_split(data.x)
...
#计算shap
sim = model_gsl(data.x, original_adj)
cls_out = model_cls_data(data.x, data.edge_index)
confidence = compute_confidence(cls_out)
adj = model_ungsl(sim, confidence)
edge_index = (adj > config['threshold']).nonzero().t()
omics_shap=shap_analysis(data,model_cls_data,edge_index)
print(omics_shap)
outs = []
for model in models:
    _, model_cls,_ = model
