...
def s_train_test_split(data, train_ratio):
    ...
    positive_train_indes=positive_index[positive_perm[:positive_train_size]]
    negative_train_indes=negative_index[negative_perm[:negative_train_size]]
    positive_test_indes=positive_index[positive_perm[positive_train_size:]]
    negative_test_indes=negative_index[negative_perm[negative_train_size:]]
    train_index=torch.cat((positive_train_indes, negative_train_indes))
    test_index=torch.cat((positive_test_indes, negative_test_indes))
...
def perform_5_fold_cv(data):
    ...
    return all_models
#交叉验证
all_models_from_data=[]
for data in dataset:
    data=data.to(primary_device)
    models = perform_5_fold_cv(data)
    all_models_from_data.append(models)
model_ungsl,model_cls_data,model_gsl=train_model(data)
original_adj = torch.zeros((data.num_nodes, data.num_nodes), device=primary_device)
original_adj[data.edge_index[0], data.edge_index[1]] = 1
outs = []
for all_models in all_models_from_data:
    for model in all_models:
        ...
        edge_index = (adj > config['threshold']).nonzero().t()
        out = model_cls(data.x, edge_index)
        outs.append(out[data.test_mask].cpu().detach().numpy())
outs=np.array(outs)
outs_mean=np.sum(outs,axis=0)/len(all_models_from_data)
